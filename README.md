...
##### 補足
*アプリケーション、アプリケーションコードおよび本READMEでは、ツリー構造を持つタスク群について、親を持たないタスク(またはこうしたタスクと連関にある一連のタスク群全体)を「プロジェクト」、プロジェクトタスクを含む子タスクを持つタスク全般または特定のタスクに対しての親を「親タスク」、親を持つタスク(プロジェクトタスク以外のタスク)全般または特定のタスクに対しての子を「子タスク」と呼ぶが、これらはタスクを管理するtasksテーブルにおいては等価である。
