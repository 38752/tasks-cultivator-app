...
# アプリケーション概要
特に、見通しを立てる事が難しいプロジェクトにおけるタスク管理を目的としたアプリケーション。
...
ツリー構造によるタスク管理において致命的な欠点となるのが、期日管理の難しさである。タスクの階層が深くなるにつれ、どのタスクにどれだけのタスクが紐づいており、それぞれの期日がいつなのかを把握することが難しくなってしまう(もし完璧に把握することができるのならそもそもタスク管理ツールは必要ない)。そこで、それぞれのタスクに期限となる日付を設定することができるようにし、それらがひと目で分かるようカレンダーに表示されるようにした。

##### 補足
*アプリケーション、アプリケーションコードおよび本READMEでは、ツリー構造を持つタスク群について、親を持たないタスク(またはこうしたタスクと連関にある一連のタスク群全体)を「プロジェクト」、プロジェクトタスクを含む子タスクを持つタスク全般を「親タスク」、親を持つタスク(プロジェクトタスク以外のタスク)全般を「子タスク」と呼ぶが、これらはタスクを管理するtasksテーブルにおいては等価である。
